...
**方法A：ZIPでまとめてダウンロード（Gitが入っていない場合）**

1. GitHubページ右上の「Code」ボタン → 「Download ZIP」をクリック
2. ダウンロードした ZIP を解凍し、任意のフォルダ（例：`C:\LLM`）に置く

**方法B：Git コマンドでクローン（Git インストール済みの場合）**
...
`docker-compose.yml` があるフォルダでターミナル（コマンドプロンプト）を開き、次のコマンドを実行します。

```bash
docker compose up -d
```

> **ターミナルの開き方（Windows）**: `docker-compose.yml` があるフォルダをエクスプローラーで開き、アドレスバーに `cmd` と入力してEnterを押すと、そのフォルダでコマンドプロンプトが開きます。

初回は Open WebUI のイメージダウンロードに数分かかります（進捗がターミナルに表示されます）。
